...
export function MicroserviceDeliveryChecklist(props: Props) {
  ...
  const blockedKeys = new Set(props.blockedServiceKeys);
  return (
    <div className={styles.deliveryChecklist}>
      ...
      {services.length ? <div className={styles.deliveryList}>{services.map((service) => <DeliveryItem blocked={blockedKeys.has(service.serviceKey)} key={service.projectId} retrying={props.retryingProjectId === service.projectId} service={service} onRetry={props.onRetry} />)}</div> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="当前业务平台暂无注册微服务" />}
    </div>
  );
}

function DeliveryItem({ blocked, service, retrying, onRetry }: { blocked: boolean; service: RegisteredDeploymentMicroservice; retrying: boolean; onRetry: (projectId: string) => void }) {
  ...
  return (
    <div className={`${styles.deliveryItem} ${blocked ? styles.deliveryBlocked : ""}`}>
      ...
      <Space size={6} wrap>
        ...
        {blocked ? <Tag color="error">后端阻断</Tag> : null}
