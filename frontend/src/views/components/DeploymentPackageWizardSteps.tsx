...
import type { DeployMode, DeploymentPackageOptions, DeploymentServiceOption, ImageExportEnvironmentCheck, PackagePreview, ProjectProfile, SourceEnv } from "../../api/deploymentPackages";
import { businessOptionValue, TargetDraft } from "./deploymentPackageUtils";
...
export function PlatformServicesStep(props: WizardStepProps) {
  return <div className={styles.wizardGrid}><ServiceGroup title="业务平台服务" items={props.businessOptionsForSourceEnv} value={props.businessServices} icon="ri-apps-2-line" empty="当前来源环境暂无已注册业务平台" onChange={props.onBusinessChange} /><ServiceGroup title="基础平台服务" items={props.platformOptionsForSourceEnv} value={props.platformServices} icon="ri-server-line" empty="当前来源环境暂无真实基础平台服务" onChange={props.onPlatformChange} onRequiredPlatformClick={props.onRequiredPlatformClick} /></div>;
}
...
export function ConfirmStep(props: WizardStepProps) {
  return <div className={styles.wizardGrid}><div className={styles.wizardSection}><h3 className={styles.sectionTitle}>导包确认</h3><div className={styles.draftGrid}><DraftItem label="项目" value={props.selectedProject?.name || props.projectKey || "未选择"} /><DraftItem label="版本" value={props.productVersion || "未选择"} /><DraftItem label="来源环境" value={props.sourceEnv === "dev" ? "开发环境" : "测试环境"} /><DraftItem label="部署方式" value={props.deployMode === "k8s" ? "Kubernetes" : "Docker Compose"} /><DraftItem label="业务平台" value={props.selectedBusinessOptions.map((item) => item.name).join("、") || "未选择"} /><DraftItem label="基础平台" value={props.selectedPlatformOptions.map((item) => item.name).join("、") || "未选择"} /><DraftItem label="数据库" value={props.selectedDatabaseOption?.name || props.database || "未选择"} /><DraftItem label="镜像模式" value={props.targetDraft.imageMode === "image-manifest" ? "镜像清单" : "镜像归档"} /><DraftItem label="目标环境" value={props.targetDraft.env || "-"} /><DraftItem label="目标域名" value={props.targetDraft.domain || "-"} /><DraftItem label="命名空间" value={props.targetDraft.namespacePrefix || "-"} /><DraftItem label="StorageClass" value={props.targetDraft.storageClass || "集群默认"} /></div></div><div className={styles.wizardSection}><div className={styles.panelTitleRow}><h3 className={styles.sectionTitle}>预览摘要</h3><Button size="small" icon={<i className="ri-refresh-line" />} loading={props.previewing} onClick={props.onRefreshPreview}>刷新</Button></div>{props.preview ? <PreviewSnapshot preview={props.preview} /> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无预览" />}</div></div>;
}

function ServiceGroup({ title, items, value, icon, empty, onChange, onRequiredPlatformClick }: { title: string; items: DeploymentServiceOption[]; value: string[]; icon: string; empty: string; onChange: (values: Array<string | number | boolean>) => void; onRequiredPlatformClick?: (event: CheckboxChangeEvent) => void }) {
  return <div className={styles.wizardSection}><h3 className={styles.sectionTitle}>{title}</h3><Checkbox.Group className={styles.serviceGrid} value={value} onChange={onChange}>{items.map((item) => <Checkbox key={`${item.sourceEnv}-${item.key}-${item.profile || "default"}`} value={title.startsWith("业务") ? businessOptionValue(item) : item.key} disabled={item.required} onChange={item.required ? onRequiredPlatformClick : undefined}><span className={styles.serviceItem}><span className={styles.serviceMain}><i className={icon} /><span className={styles.serviceName}>{item.name}</span></span><span className={styles.muted}>{item.required ? "必选" : item.namespace || item.profile || item.namespaceGroup}</span></span></Checkbox>)}</Checkbox.Group>{items.length === 0 ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={empty} /> : null}</div>;
}
