...
def _install_ps1(default_mode: str = "k8s") -> str:
    return (
        ...
        "    & docker load -i $archivePath | Out-Host\n"
        "    if ($image.sourceRef -and $image.targetRef -and $image.sourceRef -ne $image.targetRef) {\n"
        "      & docker tag $image.sourceRef $image.targetRef\n"
        "    }\n"
        "  }\n"
